...
public class WebSessionUtil {
    ...
    public static void whenLogin(User account, List<Permission> permissionList) {
        WebSessionModel sessionModel = WebSessionModel.assemblyData(
    ...
    */
    ...
    /** 获取用户信息 */
    private static WebSessionModel getSessionInfo() {
        return (WebSessionModel) RequestUtils.getSession().getAttribute(USER);
    }

    /** 获取用户信息. 没有则使用默认信息 */
    private static WebSessionModel getSessionInfoWithDefault() {
        WebSessionModel sessionModel = getSessionInfo();
