...
@ToString
class WebSessionModel implements Serializable {
    private static final long serialVersionUID = 1L;

    /** 默认未登录用户的 id */
    private static final Long DEFAULT_ID = 14L;
    /** 默认未登录用户的 name */
...
//        }
    ...
    /*static WebSessionModel assemblyData(WebSessionModel sessionModel, String domain,
        ...
        WebSessionModel session = JsonUtil.convert(account, WebSessionModel.class);
    ...
    /** 未登录时的默认用户信息 */
    static WebSessionModel defaultUser() {
        return new WebSessionModel().setId(DEFAULT_ID).setName(DEFAULT_NAME);
    }
